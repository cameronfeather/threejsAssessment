...
class SceneController {
  ...
  move(axis, distance) {
    const currentCube = this.sceneObjects[this.sceneObjects.length - 1];
    currentCube.mesh.rotation.y = this.controller.rotation.y;
    
    axis == 'x' ? currentCube.mesh.translateX(distance): 
    axis == 'y' ? currentCube.mesh.translateY(distance):
    axis == 'z' ? currentCube.mesh.translateZ(distance):
    currentCube.mesh.translateZ(0);
    currentCube.mesh.rotation.y = 0;
  }
